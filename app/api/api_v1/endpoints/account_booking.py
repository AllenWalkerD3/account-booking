...
async def get_account_books(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def create_account_book(book: schemas.account_book.AccountBookCreate, db: Session = Depends(get_db)):
    return models.booking.AccountBook.create_account_book(db=db, book=book)


@router.get("/account-type/{account_type_id}", response_model=schemas.account_book.AccountType)
...
async def get_account_types(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def create_account_book(account_type: schemas.account_book.AccountTypeCreate, db: Session = Depends(get_db)):
    return models.booking.AccountType.create_account_type(db=db,account_type=account_type)
